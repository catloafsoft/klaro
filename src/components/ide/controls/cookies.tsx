...
const CookieForm = ({updateConfig, config, cookie, t}: any) => {
    const updateCookieConfig = (key: Array<string | number | null>, value: unknown) => {
    ...
    return <div className="cm-cookie-form">
        ...
        <div className="cm-config-controls">
            ...
            <button type="button" className="cm-control-button cm-delete" onClick={() => updateConfig(['services', config._id, 'cookies', cookie._id], null)}>
                {t(['cookies', 'delete'])}
            </button>
        </div>
    </div>
...
export const Cookies = ({t,
    ...
    let cookies = config.cookies.map((cookie: any) => <CookieForm t={t} config={config} key={cookie._id} cookie={cookie} updateConfig={updateConfig} />)
    if (cookies.length === 0)
    ...
    return <div className="cm-cookie-config">
        ...
        <div className="cm-config-controls">
            <fieldset>
                <button type="button" className="cm-control-button" onClick={() => updateConfig(['services', config._id, 'cookies', null], {})}>
                    {t(['cookies', 'add'])}
                </button>
            </fieldset>
        </div>
